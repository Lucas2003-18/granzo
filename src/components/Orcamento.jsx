    ...
    <div style={{padding:16,paddingBottom:100}}>
      ...
      <div style={{...CARD,background:"rgba(61,186,111,0.07)",border:"1px solid rgba(61,186,111,0.2)",marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          <div style={{fontSize:13,color:"#3DBA6F",fontWeight:700}}>Total orçado</div>
          <div style={{fontSize:15,fontWeight:800,color:"#3DBA6F"}}>{hide?"••••":fmt(totalOrc)}</div>
        </div>
        <Bar pct={totalOrc>0?(totalGasto/totalOrc)*100:0} color={totalGasto>totalOrc?"#E05252":"#3DBA6F"}/>
        <div style={{fontSize:11,color:"#536057",marginTop:4}}>{hide?"••••":fmt(totalGasto)} gasto de {hide?"••••":fmt(totalOrc)} orçado</div>
      </div>
      {cats.map((cat,idx)=>{
        ...
        const borderColor=over?"#E05252":exact?"#3DBA6F":cat.color;
        const barColor=over?"#E05252":exact?"#3DBA6F":pct>75?"#E8A832":cat.color;
        return <div key={cat.id} style={{...CARD,padding:14,borderLeft:`3px solid ${borderColor}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:700,color:"#DDE8DF"}}>{cat.label}</div>
              {over&&<div style={{fontSize:11,color:"#E05252"}}>⚠️ Excedido em {hide?"••••":fmt(spent-cat.budget)}</div>}
              {exact&&<div style={{fontSize:11,color:"#3DBA6F"}}>✓ Limite atingido exatamente</div>}
            </div>
            <span style={{fontSize:12,color:over?"#E05252":exact?"#3DBA6F":"#536057"}}>{hide?"••••":fmt(spent)}</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            <span style={{fontSize:12,color:"#536057",flexShrink:0}}>Limite R$</span>
          ...
          {cat.budget>0&&<>
            ...
            <div style={{fontSize:11,color:"#536057",marginTop:4}}>
              {rawPct.toFixed(0)}% · {over?<span style={{color:"#E05252"}}>{hide?"••••":fmt(spent-cat.budget)} acima</span>:exact?<span style={{color:"#3DBA6F"}}>no limite ✓</span>:<span>{hide?"••••":fmt(cat.budget-spent)} restante</span>}
            </div>
          </>}
        </div>;
      })}
